(function($, fnFrontend){
	...
	var Frenifynekoin = {
		...
		init: function() {
			...
			if($('body').hasClass('admin-bar')){
				Frenifynekoin.isAdmin 		= true;
				Frenifynekoin.adminBarH 	= $('#wpadminbar').height();
			}

			var widgets = {
				'frel-gallery.default' : Frenifynekoin.allGalleryFunctions,
				'frel-down-button.default' : Frenifynekoin.downButtonFunction,
				'frel-video.default' : Frenifynekoin.videoFunction,
				'frel-buttons.default' : Frenifynekoin.ImgToSVG,
				'frel-team-member.default' : Frenifynekoin.teamMemberFunction,
				'frel-hero-header.default' : Frenifynekoin.heroHeader,
				'frel-counter-list.default' : Frenifynekoin.counter,
				'frel-roadmaps.default' : Frenifynekoin.roadmapSwiper,
			};
		...
		roadmapSwiper: function(){
			...
			section.each(function(){
				...
				Frenifynekoin.roadmapStep(mySwiper,step,widthParts);
				
				mySwiper.on('slideChange', function () {
					Frenifynekoin.roadmapStep(this,step,widthParts);
				});
			});	
		},
			...
			if(slider.length){
				slider.each(function(){
					...
					var myInterval 		= setInterval(function(){
						activeIndex++;
						activeIndex 	= Frenifynekoin.sliderDo(sliderTop,sliderBottom,activeIndex);
					},speed);
		...
		hh_bendedCarousel: function(){
			...
			if(slider.length){
				...
				slider.each(function(){
					...
					Frenifynekoin.floww_change_slide(1,element);

					// item click function
					children.on('click',function(){
						var el 			= $(this);
						var index		= el.index() + 1;
						Frenifynekoin.floww_change_slide(index,element);
					});
					Frenifynekoin.floww_start_autoplay(ul,element);
				});
			}
		},
		...
		floww_change_slide: function(index, element){
			...
			if(!el.hasClass('current')){
				...
				Frenifynekoin.floww_calc_call(element);
			}
		},
		...
		floww_calc_call: function(element){
			if(typeof element === 'undefined'){
				$('.frenify_cards_gallery').each(function(){
					element = $(this);
					Frenifynekoin.flow_calc(element);
				});
			}else{
				Frenifynekoin.flow_calc(element);
			}
		...
		heroHeader: function(){
			Frenifynekoin.videoFunction();
			Frenifynekoin.hh_3dcarousel();
			Frenifynekoin.hh_bendedCarousel();
			Frenifynekoin.hh_simpleCarousel();
			Frenifynekoin.hh_fullSlider();
			Frenifynekoin.BgImg();
		},
		...
		downButtonFunction: function(){
			$('.nekoin_fn_down').on('click',function(){
				var e 		= $(this),
					href 	= e.attr('href');
		...
		allGalleryFunctions: function(){
			Frenifynekoin.lightGallery();
			Frenifynekoin.portalGallery();
			Frenifynekoin.justifiedGallery();
			Frenifynekoin.galleryMasonry();
			Frenifynekoin.BgImg();
			Frenifynekoin.gallerySlider();
			Frenifynekoin.collageCarousel();
			Frenifynekoin.isotopeFunction();
			Frenifynekoin.inlineStyle();
			Frenifynekoin.gallerySlider2();
			setTimeout(function(){
				Frenifynekoin.isotopeFunction();
			},2000);
		},
	...
	$( window ).on( 'elementor/frontend/init', Frenifynekoin.init );
	
	
	$( window ).on('resize',function(){
		Frenifynekoin.isotopeFunction();
		Frenifynekoin.floww_calc_call();
		setTimeout(function(){
			Frenifynekoin.isotopeFunction();
		},700);
	});
